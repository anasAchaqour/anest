...
@section('content')
    ...
    <div class="w-100 h-100 d-flex align-items-center justify-content-center formDiv">
        <div class="w-75">
            <form class="w-100" action="/clients/{{ $user->id }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT') <!-- Use PUT method for update -->
                <div class="p-3 text-center" style="margin-bottom: 20px">
                    <h3 class="modal-title" id="exampleModalLabel">Edit infos of this client</h3>
                </div>
                <!-- Your form content -->
                ...
                <div class="input-group mb-3">
                    ...
                    <input type="text" class="form-control {{ $errors->has('contact') ? 'is-invalid' : '' }}"
                        placeholder="enter the Phone number" aria-label="" aria-describedby="basic-addon1" name="contact" value="{{ old('contact', $client->contact) }}">
                    @error('contact')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="input-group mb-3">
                    ...
                    <label class="input-group-text" for="inputGroupFile02">Upload Client picture</label>
                    @error('user_pic')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
