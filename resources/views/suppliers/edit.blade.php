...
@section('content')
    ...
    <div class="w-100 h-100 d-flex align-items-center justify-content-center formDiv">
        <div class="w-75">
            <form class="w-100" action="/suppliers/{{ $supplier->id }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('put')
                ...
                <div class="form-floating mb-3">
                    <textarea class="form-control {{ $errors->has('address') ? 'is-invalid' : '' }}" placeholder="Leave a comment here"
                        id="floatingTextarea" name="address">{{ $supplier->address }}</textarea>
                ...
                <div class="input-group mb-3">
                    ...
                    <input type="text" class="form-control {{ $errors->has('phone_number') ? 'is-invalid' : '' }}"
                        ...
                        name="phone_number" value="{{ $supplier->phone_number }}">
                    @error('phone_number')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
