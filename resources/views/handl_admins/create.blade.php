...
@section('content')
    ...
    <div class="w-100 h-100 d-flex align-items-center justify-content-center formDiv">
        <div class="w-75">
            <form class="w-100" action="/clients" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="input-group mb-3">
                    ...
                    <input type="text" class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}"
                        placeholder="name of client" aria-label="Username" aria-describedby="basic-addon1" name="name" value="{{ old('name') }}">
                    @error('name')
                ...
                <div class="input-group mb-3">
                    ...
                    <input type="text" class="form-control {{ $errors->has('lastName') ? 'is-invalid' : '' }}"
                        placeholder="last Name of client" name="lastName" value="{{ old('lastName') }}">
                    @error('lastName')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
